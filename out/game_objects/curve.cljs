...
(defn ->Curve
  ...
  ([scene]
   (js/Phaser.GameObjects.Curve. (clj->phaser scene)))
  ([scene x]
   (js/Phaser.GameObjects.Curve. (clj->phaser scene)
                                 (clj->phaser x)))
  ([scene x y]
  ...
  ([scene x y curve]
   (js/Phaser.GameObjects.Curve. (clj->phaser scene)
                                 ...
                                 (clj->phaser y)
                                 (clj->phaser curve)))
  ([scene x y curve fill-color]
   (js/Phaser.GameObjects.Curve. (clj->phaser scene)
                                 (clj->phaser x)
                                 (clj->phaser y)
                                 (clj->phaser curve)
                                 (clj->phaser fill-color)))
  ([scene x y curve fill-color fill-alpha]
   (js/Phaser.GameObjects.Curve. (clj->phaser scene)
                                 (clj->phaser x)
                                 (clj->phaser y)
                                 (clj->phaser curve)
...
(defn add-listener
  ...
  Parameters:
    * curve (Phaser.GameObjects.Curve) - Targeted instance for method
...
(defn get-bottom-center
  ...
  ([curve]
   (phaser->clj
    (.getBottomCenter curve)))
  ([curve output]
   (phaser->clj
    (.getBottomCenter curve
                      (clj->phaser output))))
  ([curve output include-parent]
   (phaser->clj
    (.getBottomCenter curve
                      (clj->phaser output)
...
(defn get-world-transform-matrix
  ...
  ([curve]
   (phaser->clj
    (.getWorldTransformMatrix curve)))
  ([curve temp-matrix]
   (phaser->clj
    (.getWorldTransformMatrix curve
                              (clj->phaser temp-matrix))))
  ([curve temp-matrix parent-matrix]
   (phaser->clj
    (.getWorldTransformMatrix curve
                              (clj->phaser temp-matrix)
...
(defn set-alpha
  ...
  ([curve top-left]
   (phaser->clj
    (.setAlpha curve
               (clj->phaser top-left))))
  ([curve top-left top-right]
   (phaser->clj
    (.setAlpha curve
               (clj->phaser top-left)
               (clj->phaser top-right))))
  ([curve top-left top-right bottom-left]
   (phaser->clj
    (.setAlpha curve
               (clj->phaser top-left)
               (clj->phaser top-right)
...
(defn set-interactive
  ...
  ([curve]
   (phaser->clj
    (.setInteractive curve)))
  ([curve shape]
   (phaser->clj
    (.setInteractive curve
                     (clj->phaser shape))))
  ([curve shape callback]
   (phaser->clj
    (.setInteractive curve
                     (clj->phaser shape)
                     (clj->phaser callback))))
  ([curve shape callback drop-zone]
   (phaser->clj
    (.setInteractive curve
                     (clj->phaser shape)
                     (clj->phaser callback)
...
(defn set-stroke-style
  ...
  ([curve]
   (phaser->clj
    (.setStrokeStyle curve)))
  ([curve line-width]
   (phaser->clj
    (.setStrokeStyle curve
                     (clj->phaser line-width))))
  ([curve line-width color]
   (phaser->clj
    (.setStrokeStyle curve
                     (clj->phaser line-width)
                     (clj->phaser color))))
  ([curve line-width color alpha]
   (phaser->clj
    (.setStrokeStyle curve
                     (clj->phaser line-width)
                     (clj->phaser color)
