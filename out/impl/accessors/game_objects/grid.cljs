(ns phzr.impl.accessors.game-objects.grid)

(def grid-get-properties
  {:active "active"
   :alpha "alpha"
   :alpha-bottom-left "alphaBottomLeft"
   :alpha-bottom-right "alphaBottomRight"
   :alpha-top-left "alphaTopLeft"
   :alpha-top-right "alphaTopRight"
   :alt-fill-alpha "altFillAlpha"
   :alt-fill-color "altFillColor"
   :angle "angle"
   :blend-mode "blendMode"
   :body "body"
   :camera-filter "cameraFilter"
   :cell-height "cellHeight"
   :cell-width "cellWidth"
   :close-path "closePath"
   :data "data"
   :default-pipeline "defaultPipeline"
   :depth "depth"
   :display-height "displayHeight"
   :display-origin-x "displayOriginX"
   :display-origin-y "displayOriginY"
   :display-width "displayWidth"
   :fill-alpha "fillAlpha"
   :fill-color "fillColor"
   :height "height"
   :ignore-destroy "ignoreDestroy"
   :input "input"
   :is-filled "isFilled"
   :is-stroked "isStroked"
   :line-width "lineWidth"
   :mask "mask"
   :name "name"
   :origin-x "originX"
   :origin-y "originY"
   :outline-fill-alpha "outlineFillAlpha"
   :outline-fill-color "outlineFillColor"
   :parent-container "parentContainer"
   :path-data "pathData"
   :path-indexes "pathIndexes"
   :pipeline "pipeline"
   :render-flags "renderFlags"
   :rotation "rotation"
   :scale "scale"
   :scale-x "scaleX"
   :scale-y "scaleY"
   :scroll-factor-x "scrollFactorX"
   :scroll-factor-y "scrollFactorY"
   :show-alt-cells "showAltCells"
   :show-cells "showCells"
   :show-outline "showOutline"
   :state "state"
   :stroke-alpha "strokeAlpha"
   :stroke-color "strokeColor"
   :tab-index "tabIndex"
   :type "type"
   :visible "visible"
   :w "w"
   :width "width"
   :x "x"
   :y "y"
   :z "z"})

(def grid-set-properties
  {:active "active"
   :alpha "alpha"
   :alpha-bottom-left "alphaBottomLeft"
   :alpha-bottom-right "alphaBottomRight"
   :alpha-top-left "alphaTopLeft"
   :alpha-top-right "alphaTopRight"
   :alt-fill-alpha "altFillAlpha"
   :alt-fill-color "altFillColor"
   :angle "angle"
   :blend-mode "blendMode"
   :body "body"
   :camera-filter "cameraFilter"
   :cell-height "cellHeight"
   :cell-width "cellWidth"
   :close-path "closePath"
   :default-pipeline "defaultPipeline"
   :depth "depth"
   :display-height "displayHeight"
   :display-origin-x "displayOriginX"
   :display-origin-y "displayOriginY"
   :display-width "displayWidth"
   :fill-alpha "fillAlpha"
   :fill-color "fillColor"
   :height "height"
   :ignore-destroy "ignoreDestroy"
   :input "input"
   :is-filled "isFilled"
   :is-stroked "isStroked"
   :line-width "lineWidth"
   :mask "mask"
   :name "name"
   :origin-x "originX"
   :origin-y "originY"
   :outline-fill-alpha "outlineFillAlpha"
   :outline-fill-color "outlineFillColor"
   :parent-container "parentContainer"
   :pipeline "pipeline"
   :render-flags "renderFlags"
   :rotation "rotation"
   :scale "scale"
   :scale-x "scaleX"
   :scale-y "scaleY"
   :scroll-factor-x "scrollFactorX"
   :scroll-factor-y "scrollFactorY"
   :show-alt-cells "showAltCells"
   :show-cells "showCells"
   :show-outline "showOutline"
   :state "state"
   :stroke-alpha "strokeAlpha"
   :stroke-color "strokeColor"
   :tab-index "tabIndex"
   :type "type"
   :visible "visible"
   :w "w"
   :width "width"
   :x "x"
   :y "y"
   :z "z"})