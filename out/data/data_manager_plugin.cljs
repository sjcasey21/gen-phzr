(ns phzr.data.data-manager-plugin
  (:require [phzr.impl.utils.core :refer [clj->phaser phaser->clj]]
            [phzr.impl.extend :as ex]
            [cljsjs.phaser])
  (:refer-clojure :exclude [get set merge remove pop]))

(defn ->DataManagerPlugin
  "  Parameters:
    * scene (Phaser.Scene) - A reference to the Scene that this DataManager belongs to."
  ([scene]
   (js/Phaser.Data.DataManagerPlugin. (clj->phaser scene))))
...
(defn destroy
  ...
  ([data-manager-plugin]
   (phaser->clj
    (.destroy data-manager-plugin))))

(defn each
  "Passes all data entries to the given callback.

  Parameters:
    * data-manager-plugin (Phaser.Data.DataManagerPlugin) - Targeted instance for method
  ...
  ([data-manager-plugin callback]
   (phaser->clj
    (.each data-manager-plugin
           (clj->phaser callback))))
  ([data-manager-plugin callback context]
   (phaser->clj
    (.each data-manager-plugin
           (clj->phaser callback)
           (clj->phaser context))))
  ([data-manager-plugin callback context args]
   (phaser->clj
    (.each data-manager-plugin
           (clj->phaser callback)
           (clj->phaser context)
...
(defn get
  ...
  ([data-manager-plugin key]
   (phaser->clj
    (.get data-manager-plugin
          (clj->phaser key)))))
...
(defn merge
  ...
  Returns:  Phaser.Data.DataManager - This DataManager object."
  ([data-manager-plugin data]
   (phaser->clj
    (.merge data-manager-plugin
            (clj->phaser data))))
  ([data-manager-plugin data overwrite]
   (phaser->clj
    (.merge data-manager-plugin
            (clj->phaser data)
...
(defn query
  ...
  ([data-manager-plugin search]
   (phaser->clj
    (.query data-manager-plugin
            (clj->phaser search)))))
...
(defn set
  ...
  Returns:  Phaser.Data.DataManager - This DataManager object."
  ([data-manager-plugin key data]
   (phaser->clj
    (.set data-manager-plugin
          (clj->phaser key)
          (clj->phaser data)))))
...
(defn set-freeze
  ...
  Returns:  Phaser.Data.DataManager - This DataManager object."
  ([data-manager-plugin value]
   (phaser->clj
    (.setFreeze data-manager-plugin
                (clj->phaser value)))))