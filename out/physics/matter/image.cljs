...
(defn ->Image
  ...
  ([world x y texture]
   (js/Phaser.Physics.Matter.Image. (clj->phaser world)
                                    (clj->phaser x)
                                    (clj->phaser y)
                                    (clj->phaser texture)))
...
(defn add-listener
  ...
  Parameters:
    * image (Phaser.Physics.Matter.Image) - Targeted instance for method
...
(defn apply-force-from
  ...
  Returns:  Phaser.GameObjects.GameObject - This Game Object."
  ([image position force]
   (phaser->clj
    (.applyForceFrom image
                     (clj->phaser position)
                     (clj->phaser force)))))
...
(defn set-existing-body
  ...
  ([image body]
   (phaser->clj
    (.setExistingBody image
                      (clj->phaser body))))
  ([image body add-to-world]
   (phaser->clj
    (.setExistingBody image
                      (clj->phaser body)
...
(defn set-trapezoid
  ...
  Returns:  Phaser.GameObjects.GameObject - This Game Object."
  ([image width height slope options]
   (phaser->clj
    (.setTrapezoid image
                   (clj->phaser width)
                   (clj->phaser height)
